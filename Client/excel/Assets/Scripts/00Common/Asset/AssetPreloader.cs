...
public class CResPreloader : MonoSingleton<CResPreloader>
{
	...
	public void AddRes(string resFullPath, bool mustExist = false, int num = 1, ResExtrator extractor = null, int extData = 0, List<string> extDataList = null, ResType resType = ResType.OBJECT, System.Type t = null)
    {
        ...
        {
            LogManager.Instance().LogWarning("Preloading has begun, you can not add resource during procedural!");
            return;
        }
    ...
    public int DoPreLoad()
    {
        ...
        if (null == assetLoader)
        {
            LogManager.Instance().LogErrorFormat( "Get AssetLoader has failed!");
            return 0;
        }
	...
	public int DoPreLoadAsync(bool savePriority=false, bool useSync=false)
    {
        ...
        for (int i = m_ProcessCount; i < m_ResPreloadTbl.Count; ++i)
        {
			...
			//LogManager.Instance().LogErrorFormat("preload res:{0} num:{1}", curRes.m_FullPath, curRes.num);
			int num = curRes.num;
			num = 1;
			...
			if (curRes.resType == ResType.RES/*curRes.m_FullPath.StartsWith("Sound")*/)
			{
				...
				CGameObjectPool.Instance().PrepareGameObject(curRes.m_FullPath, enResourceType.BattleScene, num);
			...
			if (!useSync)
			{
				...
				{
					m_ProcessCount = i;
					LogManager.Instance().LogFormat("Step preload return {0}%.",(int)((float)m_ProcessCount / m_ResPreloadTbl.Count * 100.0f));
		...
		if (writeFile)
		{
			...
			ExceptionManager.Instance().PrintPreloadRes(contents);

			contents.Clear();
            var cachedID = new List<string>(0);// GameClient.PreloadManager.cachedResID; TODO;
			contents.Add("RES ID---------------------------------");
