...
public class CFileManager
{
    ...
    public static bool CreateDirectory(string directory)
    {
        ...
        while (true)
        {
            ...
            catch (Exception exception)
            {
                ...
                if (num >= 3)
                {
                    Logger.Log("Create Directory " + directory + " Error! Exception = " + exception.ToString());
    ...
    public static bool DeleteDirectory(string directory)
    {
        ...
        while (true)
        {
            ...
            catch (Exception exception)
            {
                ...
                if (num >= 3)
                {
                    Logger.Log("Delete Directory " + directory + " Error! Exception = " + exception.ToString());
    ...
    public static bool DeleteFile(string filePath)
    {
        ...
        while (true)
        {
            ...
            catch (Exception exception)
            {
                ...
                if (num >= 3)
                {
                    Logger.Log("Delete File " + filePath + " Error! Exception = " + exception.ToString());
    ...
    public static int GetFileLength(string filePath)
    {
        ...
        while (true)
        {
            ...
            catch (Exception exception)
            {
                ...
                if (num >= 3)
                {
                    Logger.Log("Get FileLength of " + filePath + " Error! Exception = " + exception.ToString());
                    return 0;
                }
            }
        }
    ...
    public static bool IsFileExist(string filePath)
    {
        ...
        return File.Exists(filePath);
#elif UNITY_ANDROID
        return LibZip.LibZipFileReader.FileExist(filePath);
#else
        return File.Exists(filePath);
#endif
    }
    ...
    {
        if (LibZip.LibZipFileReader.FileExist(filePath))
        {
            byte[] buff = null;
            long buffLen = 0;
            if (LibZip.LibZipFileReader.Read(filePath, ref buff, ref buffLen))
            {
                return buff;
            }
        }
    ...
    public static byte[] ReadFile(string filePath)
    {
        if (IsFileExist(filePath))
        {
            ...
            do
            {
                ...
                {
                    Logger.Log(string.Concat(new object[] { "Read File ", filePath, " Error! Exception = ", exception.ToString(), ", TryCount = ", num }));
                    buffer = null;
                }
            ...
            while (num < 3);
            Logger.Log(string.Concat(new object[] { "Read File ", filePath, " Fail!, TryCount = ", num }));
    ...
    public static bool WriteFile(string filePath, byte[] data, int offset, int length)
    {
        ...
        while (true)
        {
            ...
            catch (Exception exception)
            {
                ...
                if (num >= 3)
                {
                    Logger.Log("Write File " + filePath + " Error! Exception = " + exception.ToString());
                    DeleteFile(filePath);
                    //s_delegateOnOperateFileFail(filePath, enFileOperation.WriteFile);
                    return false;
                }
