...
Shader "HeroGo/Surface/General_Rim_Flash_Alpha"
{
	Properties
	{
	...
	SubShader
	{
		...
		Pass
		{
			...
			fixed4 frag(v2f IN) : SV_Target
			{
				...
				real rim = 1.0 - saturate(dot(Vn, Nn));
				real factor = pow(saturate(rim - _RimLightOffset), _RimLightPower) * IN.TintFactor;
