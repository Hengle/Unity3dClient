...
namespace ProtoBuf.CodeGenerator
{
    public sealed class CommandLineOptions
    {
        ...
        public string outClassPrefix;
        public Dictionary<string, string> FieldCryptic; 
            ...
            try
            {
                ...
                try
                {
                    ...
                    else if (!string.IsNullOrEmpty(this.OutPath))
                    {
                        ...
                        var fileName = Path.GetFileName(this.OutPath);
                        UnityEngine.Debug.LogFormat("convert <color=#00ff00>{0}</color> succeed !", fileName);
                    }
                    else if (string.IsNullOrEmpty(this.OutPath))
                    {
        ...
        private static void ProcessFileDescriptorSet(FileDescriptorSet set, CommandLineOptions options)
        {
            if (set == null || set.file == null || set.file.Count == 0)
                return;

            List<FileDescriptorProto> filedProtoList = set.file;
            for (int i = 0; i < filedProtoList.Count; i++)
            {
                List<DescriptorProto> dProtoList = filedProtoList[i].message_type;
                if (dProtoList != null && dProtoList.Count > 0)
                {
                    for (int j = 0; j < dProtoList.Count; j++)
                    {
                        if (!string.IsNullOrEmpty(options.outClassPrefix))
                        {
                            dProtoList[j].name = options.outClassPrefix + dProtoList[j].name;
                        }
                        List<FieldDescriptorProto> fielddProto = dProtoList[j].field;
                        for (int index = 0; index < fielddProto.Count; index++)
                        {
                            if (fielddProto[index] != null && options.FieldCryptic != null 
                                && options.FieldCryptic.ContainsKey(fielddProto[index].name))
                            {
                                fielddProto[index].cryptic = true;
                            }
                        }
                    }
                }
