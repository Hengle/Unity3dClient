...
namespace XLua
{
    public partial class ObjectTranslator
    {
        ...
        public void PushGameClientFrameTypeID(RealStatePtr L, GameClient.FrameTypeID val)
        {
            if (GameClientFrameTypeID_TypeID == -1)
            {
			    bool is_first;
                GameClientFrameTypeID_TypeID = getTypeId(L, typeof(GameClient.FrameTypeID), out is_first);
				
				if (GameClientFrameTypeID_EnumRef == -1)
				{
				    Utils.LoadCSTable(L, typeof(GameClient.FrameTypeID));
				    GameClientFrameTypeID_EnumRef = LuaAPI.luaL_ref(L, LuaIndexes.LUA_REGISTRYINDEX);
				}
				
            }
			
			if (LuaAPI.xlua_tryget_cachedud(L, (int)val, GameClientFrameTypeID_EnumRef) == 1)
            {
			    return;
			}
			
            IntPtr buff = LuaAPI.xlua_pushstruct(L, 4, GameClientFrameTypeID_TypeID);
            if (!CopyByValue.Pack(buff, 0, (int)val))
            {
                throw new Exception("pack fail fail for GameClient.FrameTypeID ,value="+val);
            }
			
			LuaAPI.lua_getref(L, GameClientFrameTypeID_EnumRef);
        ...
        public void Get(RealStatePtr L, int index, out GameClient.FrameTypeID val)
        {
		    LuaTypes type = LuaAPI.lua_type(L, index);
            if (type == LuaTypes.LUA_TUSERDATA )
            {
			    if (LuaAPI.xlua_gettypeid(L, index) != GameClientFrameTypeID_TypeID)
				{
				    throw new Exception("invalid userdata for GameClient.FrameTypeID");
				}
                ...
                {
                    throw new Exception("unpack fail for GameClient.FrameTypeID");
                }
				val = (GameClient.FrameTypeID)e;
                
            }
            else
            {
                val = (GameClient.FrameTypeID)objectCasters.GetCaster(typeof(GameClient.FrameTypeID))(L, index, null);
            }
        }
		
        public void UpdateGameClientFrameTypeID(RealStatePtr L, int index, GameClient.FrameTypeID val)
        {
            ...
            if (LuaAPI.lua_type(L, index) == LuaTypes.LUA_TUSERDATA)
            {
                ...
                if (!CopyByValue.Pack(buff, 0,  (int)val))
                {
                    throw new Exception("pack fail for GameClient.FrameTypeID ,value="+val);
                }
            }
        ...
        int GameClientFrameLayer_TypeID = -1;
		int GameClientFrameLayer_EnumRef = -1;
        
        public void PushGameClientFrameLayer(RealStatePtr L, GameClient.FrameLayer val)
        {
            if (GameClientFrameLayer_TypeID == -1)
            {
			    bool is_first;
                GameClientFrameLayer_TypeID = getTypeId(L, typeof(GameClient.FrameLayer), out is_first);
				
				if (GameClientFrameLayer_EnumRef == -1)
				{
				    Utils.LoadCSTable(L, typeof(GameClient.FrameLayer));
				    GameClientFrameLayer_EnumRef = LuaAPI.luaL_ref(L, LuaIndexes.LUA_REGISTRYINDEX);
				}
				
            }
			
			if (LuaAPI.xlua_tryget_cachedud(L, (int)val, GameClientFrameLayer_EnumRef) == 1)
            {
			    return;
			}
			
            IntPtr buff = LuaAPI.xlua_pushstruct(L, 4, GameClientFrameLayer_TypeID);
            if (!CopyByValue.Pack(buff, 0, (int)val))
            {
                throw new Exception("pack fail fail for GameClient.FrameLayer ,value="+val);
            }
			
			LuaAPI.lua_getref(L, GameClientFrameLayer_EnumRef);
			LuaAPI.lua_pushvalue(L, -2);
			LuaAPI.xlua_rawseti(L, -2, (int)val);
			LuaAPI.lua_pop(L, 1);
        ...
        public void Get(RealStatePtr L, int index, out GameClient.FrameLayer val)
        {
		    LuaTypes type = LuaAPI.lua_type(L, index);
            if (type == LuaTypes.LUA_TUSERDATA )
            {
			    if (LuaAPI.xlua_gettypeid(L, index) != GameClientFrameLayer_TypeID)
				{
				    throw new Exception("invalid userdata for GameClient.FrameLayer");
				}
                ...
                if (!CopyByValue.UnPack(buff, 0, out e))
                {
                    throw new Exception("unpack fail for GameClient.FrameLayer");
                }
				val = (GameClient.FrameLayer)e;
                
            }
            else
            {
                val = (GameClient.FrameLayer)objectCasters.GetCaster(typeof(GameClient.FrameLayer))(L, index, null);
            }
        }
		
        public void UpdateGameClientFrameLayer(RealStatePtr L, int index, GameClient.FrameLayer val)
        {
            ...
            if (LuaAPI.lua_type(L, index) == LuaTypes.LUA_TUSERDATA)
            {
                ...
                if (!CopyByValue.Pack(buff, 0,  (int)val))
                {
                    throw new Exception("pack fail for GameClient.FrameLayer ,value="+val);
                }
            }
	...
	public partial class StaticLuaCallbacks
    {
	    internal static bool __tryArrayGet(Type type, RealStatePtr L, ObjectTranslator translator, object obj, int index)
		{
			...
			else if (type == typeof(GameClient.FrameTypeID[]))
			{
				...
				translator.PushGameClientFrameTypeID(L, array[index]);
				return true;
			}
